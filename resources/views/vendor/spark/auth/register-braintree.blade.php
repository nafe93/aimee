            ...
            <div class="row" v-show="selectedPlan && selectedPlan.price > 0">
                <div class="col-md-8 col-md-offset-2">
                    <div class="panel panel-default">
                        <div class="panel-heading"><i class="fa fa-btn fa-credit-card"></i>{{ trans('auth/register-braintree.billing-information') }}</div>

                        <div class="panel-body">
                            <!-- Generic 500 Level Error Message / Stripe Threw Exception -->
                            <div class="alert alert-danger" v-if="registerForm.errors.has('form')">
                                {{ trans('auth/register-braintree.trouble-validating-card') }}
                            </div>
                            ...
                            <form class="form-horizontal" role="form">
                                ...
                                <div class="form-group" :class="{'has-error': registerForm.errors.has('coupon')}" v-if="query.coupon">
                                    <label for="number" class="col-md-4 control-label">{{ trans('auth/register-braintree.coupon-code') }}</label>
                                    ...
                                    <div class="col-sm-6 col-sm-offset-4">
                                        <div class="checkbox">
                                            <label>
                                                ...
                                                {{ trans('auth/register-braintree.i-accept-the') }} <a href="/terms" target="_blank">{{ trans('auth/register-braintree.terms-of-service') }}</a>
                                    ...
                                    <div class="col-sm-6 col-sm-offset-4">
                                        <button type="submit" class="btn btn-primary" :disabled="registerForm.busy">
                                            <span v-if="registerForm.busy">
                                                <i class="fa fa-btn fa-spinner fa-spin"></i>{{ trans('auth/register-braintree.registering') }}
                                            </span>

                                            <span v-else>
                                                <i class="fa fa-btn fa-check-circle"></i>{{ trans('auth/register-braintree.register') }}
                                            </span>
                                        </button>
                                    </div>
                                </div>
